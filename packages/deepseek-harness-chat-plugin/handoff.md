...
- 新会话的“过程详情”默认隐藏；已有会话仍读取各自已经持久化的显隐偏好，不会被默认值覆盖。
- 浏览器标签页 favicon 会按“任意会话运行中 > 存在可见未读 > 普通”切换：运行中在原图标右下角叠加蓝色静态省略号；无运行中会话但存在未归档、非空白、非 subagent 的未读时叠加红点，否则恢复原图标；工作区归档状态变化会立即刷新 favicon，插件卸载时也会恢复。
...
- 会话过程默认隐藏调整已运行 `pnpm run check`，类型检查、5 个测试文件（6 个测试）和生产构建均通过；重启 `http://127.0.0.1:3081` 后，`agent-browser` 打开未保存过程偏好的会话，观察到开关为“显示工具调用、思考与上下文等过程信息”且 `aria-checked=false`，Chat 中没有展示完整过程行；未发送模型请求。
- favicon 等待省略号与红点可见范围调整已运行 `pnpm run check`，类型检查、5 个测试文件（6 个测试）和生产构建均通过；本次未新增测试用例，也未做浏览器自动化验证。
