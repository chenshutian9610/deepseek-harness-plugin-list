 ...
 *
 * Forked from `@deepseek-ai/dsh-mcp-client@0.1.0-rc.8` at DeepSeek Harness
 * revision `141eb6fef83422698aef7a981029e843e8161534`. The Web-owned change scopes
 * live `serverName` reservations to `ctx.agent ?? ctx.root`.
 *
 * @module deepseek-harness-web-project-mcp/mcp-client
 */
...
/**
 * Live `serverName` reservations per registration owner. Agent-scoped clients
 * reserve against their inherited `ctx.agent`; unscoped clients reserve against
 * `ctx.root`. Different agents may expose the same public MCP namespace, while
 * duplicates inside one agent or the global layer remain configuration errors.
 */
...
export async function apply(ctx: Context, config: Config): Promise<void> {
  ...
  ctx.effect(() => {
    const reservationOwner = ctx.agent ?? ctx.root
    let names = activeServerNames.get(reservationOwner)
    if (!names) {
      names = new Set()
      activeServerNames.set(reservationOwner, names)
    }
